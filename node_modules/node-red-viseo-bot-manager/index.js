...
// Use user.id instead of user.address.conversation.id because 
// - user might be retrieve from database (proactive message) so we can't use message.addresse
// - current user conversation id can be different from database user conversation id and can't be overrided easily
const getConvId = exports.getConvId = (data) => {
    return helper.getByString(data, 'user.id', undefined)
}
