...
global.VBM.DELAYED_CALLBACK = global.VBM.DELAYED_CALLBACK || {}


let listenEnd = function (srcNode, data, srcConfig) {
    var id = getConvId(data);
	delete global.VBM.DELAYED_CALLBACK[id];
}
helper.listenEvent('endConversation', listenEnd);

const delayCallback = exports.delayCallback = (id, callback) => {
    if (!id) return;
    global.VBM.DELAYED_CALLBACK[id] = callback;
}

const hasDelayedCallback = exports.hasDelayedCallback = (id, data) => {
    if (!id) return;

    let callback = global.VBM.DELAYED_CALLBACK[id];
